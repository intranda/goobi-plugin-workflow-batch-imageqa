...
public class QaPluginManager {
    ...
    public static List<ProcessOverview> getProcesses(String stepTitle, int batchId, List<String> metadataToCheck) {
        ...
        if (metadataToCheck == null || metadataToCheck.isEmpty()) {
            ...
            sb.append(stepTitle);
            sb.append("' LEFT JOIN (SELECT object_id, property_value FROM properties WHERE object_type='process' AND property_name='QA-Status' GROUP BY object_id) p1 ON p.prozesseID = p1.object_id ");
            sb.append("LEFT JOIN (SELECT object_id, property_value FROM properties WHERE object_type='process' AND property_name='QA-Note' GROUP BY object_id) p2 ON p.prozesseID = p2.object_id ");
